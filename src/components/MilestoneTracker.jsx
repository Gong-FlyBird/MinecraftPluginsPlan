...
export default function MilestoneTracker({ plugin, highlightPluginId, onAddMilestone, onUpdateMilestone, onDeleteMilestone, onToggleTask, onAddTask, onDeleteTask, t }) {
  ...
  if (!plugin) {
    return (
      <EmptyState icon={ListTodo} title={t('milestone.selectPlugin')}
        description={t('milestone.subtitle')} />
    );
  }
    ...
    <div className={`fade-in ${isHighlighted ? 'animate-highlight' : ''}`}>
      ...
      <GlassPanel className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-hermes-text-muted">{t('milestone.total')}</span>
      ...
      {milestones.length === 0 ? (
        <EmptyState icon={ListTodo} title={t('app.empty')} description={t('milestone.noTasks')} />
      ) : (
              ...
              <GlassPanel key={m.id} className="!p-0 overflow-hidden">
                <div className="flex items-center gap-3 px-5 py-4 cursor-pointer hover:bg-hermes-gold/[0.06] transition-colors"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3">
                      <h3 className="text-sm font-semibold text-hermes-text">{m.title || t('milestone.noPlugin')}</h3>
                      {m.deadline && <span className="text-[10px] text-hermes-text-muted/50 flex items-center gap-1"><Calendar size={10} /> {formatDate(m.deadline)}</span>}
                    </div>
                    <div className="flex items-center gap-2 mt-1.5">
                      <div className="progress-bar flex-1 max-w-[200px]"><div className="progress-bar-fill" style={{ width: `${mProgress}%` }} /></div>
                      <span className="text-[10px] text-hermes-text-muted/50">{t('milestone.tasks', { done: doneTasks, total: tasks.length })}</span>
                    </div>
                  </div>
                ...
                {isExpanded && (
                  <div className="px-5 pb-4 border-t border-hermes-border/30 pt-3">
                    {tasks.length === 0 && <p className="text-xs text-hermes-text-muted/40 py-2">{t('milestone.noTasks')}</p>}
                    <div className="space-y-1.5 mb-3">
                      {tasks.map(task => (
                        <div key={task.id} className="flex items-center gap-3 group py-1.5 px-2 hover:bg-hermes-gold/[0.06] transition-colors">
                          <button onClick={() => onToggleTask(plugin.id, m.id, task.id)} className="flex-shrink-0">
                            {task.done ? <CheckCircle2 size={16} className="text-hermes-gold" /> : <Circle size={16} className="text-hermes-text-muted/40 hover:text-hermes-gold/60 transition-colors" />}
                          </button>
                    ...
                    <div className="flex gap-2">
                      <input type="text" value={newTaskText[m.id] || ''}
                      ...
                      <button onClick={() => handleAddTask(m.id)} className="glass-btn !py-1.5 !px-3 text-sm"><Plus size={14} /></button>
                    </div>
                  </div>
                )}
              </GlassPanel>
